...
/**
 * @fn
 * PdfReadTextクラスのコンストラクタ
 * @brief PdfReadTextクラスのコンストラクタ
 * @return なし
 * @sa
 * @detail PdfReadTextクラスのコンストラクタ
 */
 ...
 * pdfファイル情報の解放
 * @brief pdfファイル情報の解放
 * @param (pInfo) pdfファイル情報へのポインタ
 * @return なし
 * @sa
 * @detail pdfファイル情報の解放　リカージョンします。
 */
 ...
 * @fn
 * pdfファイル内フォント情報の解放
 * @brief pdfファイル内フォント情報の解放
 * @param (pFont) フォント情報へのポインタ
 * @return なし
 * @sa
 * @detail pdfファイル内フォント情報の解放　リカージョンします。
 */
...
/**
 * @fn
 * PdfReadTextクラスのデストラクタ
 * @brief PdfReadTextクラスのデストラクタ
 * @return なし
 * @sa
 * @detail PdfReadTextクラスのデストラクタ　クローズします
 */
PdfReadText::~PdfReadText(void)
{	PdfClose();
}
 ...
 * @fn
 * pdfファイル内のオブジェクトへシーク
 * @brief pdfファイル内のオブジェクトへシーク
 * @param (ObjNo) pdfファイル内のオブジェクト番号
 * @return シーク結果
 * @sa
 * @detail pdfファイル内のオブジェクトへシーク
 */
 ...
 * @fn
 * pdfファイル内のページへシーク
 * @brief pdfファイル内のページへシーク
 * @param (pageNo) pdfファイル内のページ番号
 * @return シーク結果
 * @sa
 * @detail pdfファイル内のページへシーク
 */
 ...
 * pdfファイルのオープン
 * @brief pdfファイルのオープン
 * @param (*pszFilePath) pdfファイルパス
 * @return ファイルハンドル
 * @sa
 * @detail pdfファイルのオープンし、内部情報を解析する
 */
 ...
 * @return なし
 * @sa
 * @detail pdfファイルのクローズ、内部情報エリアを解放する
 */
void PdfReadText::PdfClose()
{   if ( pPrintInfo != NULL )		// PageTbl
    {   freeInfo( pPrintInfo );
        pPrintInfo = NULL;
    }
    if ( pPrintInfo != NULL )		// PageTbl
 ...
 * @return 0
 * @sa
 * @detail pdfファイル情報のサーチ、最終行からxrefテーブルやtrailer等をサーチする
 */
 ...
 * Xrefテーブルの生成
 * @brief Xrefテーブルの生成
 * @param (posxref) xrefテーブルのシーク位置
 * @return 0:エラー時 / 非0:オブジェクト数
 * @sa
 * @detail Xrefテーブルの生成
 */
...
{	char	*pszToken;
    ...
    memset( pXrefTbl, 0, cntObj * sizeof( struct XREF ));
    for ( i = 0; i < cntObj; i++ )
	{	pszToken = PdfFile.getForwardToken();
 ...
 * @brief ページストリームの取得
 * @param (pageNo) ページ番号
 * @param (*pBuf) ストリーム取り出しバッファポインタ
 * @param (BufLen) ストリーム取り出しバッファサイズ
 * @return 0:エラー / BufLen:正常終了
 * @sa
 * @detail ページストリームの取得
 */
 ...
 * @fn
 * フォントのコードページの取得
 * @brief フォントのコードページの取得
 * @param (pageNo) ページ番号
 * @param (fontNo) フォント番号
 * @return 0:エラー / BufLen:正常終了
 * @sa
 * @detail フォントのコードページの取得
 */
 ...
 * @brief エスケープキャラの削除
 * @param (*pSrc) ソース文字列
 * @param (lenSrc) ソース文字列長さ
 * @return 変換後の文字数
 * @sa
 * @detail 文字化けしたエスケープキャラの削除
 */
 ...
 * ページ番号の解析
 * @brief ページ番号の解析
 * @param (**ppPrintInfo) ページ情報
 * @param (*pText) テキスト情報
 * @return なし
 * @sa
 * @detail ページ番号の解析　位置情報やフォントサイズからページ番号であろうフォント番号を解析する
 */
 ...
 * @brief ページ番号のテキストの取得
 * @param (pageNo) ページ番号
 * @param (*pSrc) ソーステキストポインタ
 * @param (lenSrc) ソーステキスト長さ
 * @param (*pDst) ディスティネーションバッファへのポインタ
 * @param (lenDst) ディスティネーションバッファサイズ
 * @param (**ppPrintInfo) ページ情報
 * @return ディスティネーションバッファにセットしたユニコード文字列サイズ
 * @sa
 * @detail ページ番号のテキストの取得
 */
...
//					stringEnd = ps;
					...
					pPageTbl[pageNo].textTbl[txtNo].Unilen = len
                            = cml.CodePageToUni( codePage, pstringStart, len, ( char * )szUniStr, sizeof( szUniStr ) / sizeof( WCHAR ));
					if ( len > 0 )
					{	if ( ppPrintInfo == NULL )
 ...
 * @return ページ情報
 * @sa
 * @detail 最小サイズフォントの検索　小さいフォントがページ番号？　ルビ？　リカージョンします
 */
 ...
 * @brief pdfページの読み込み
 * @param (pageNo) ページ番号
 * @param (*pBuf) ディスティネーションバッファへのポインタ
 * @param (BufLen) ディスティネーションバッファのサイズ
 * @return ページ文字列の長さ
 * @sa
 * @detail pdfページの読み込み　小さいフォントがページ番号？　ルビ？　リカージョンします
 */
